...
class ApnsPushSender:
    ...
    def send_development(self, device_id: str, *, event_id: str, count: int) -> None:
        if not event_id or type(count) is not int or not 1 <= count <= 100:
        ...
        payload["development_event_id"] = event_id
        self._send(device_id, payload, collapse_id="development-timeline", priority="5")
