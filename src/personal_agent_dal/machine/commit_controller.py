...
7. **Engine block transition** — on a block verdict, the controller also
   ...
   guard semantics); its duplicate-receipt semantics make the whole
...
from personal_agent_dal.storage.machine_models import CommitCapability, ExternalEffect
...
def execute_candidate_commit(
    ...
    *,
    ...
    """
    if type(now) is not int or type(current_epoch) is not int or (
        type(current_lease_epoch) is not int
    ):
        raise _invalid("controller times and epochs must be native integers")
    ...
    # A prior block landing for this identity is also a replay fence: a
    # block leaves the capability issued, so the consumed-replay branch
    # above cannot answer, and re-entering the executor would form a second
    # stray commit (or crash on the store's block-intent constraint). Ask
    # the store only when the executor work is genuinely unrecorded —
    # the block evidence row exists and is unique.
    sessions = session_factory(engine)
    with sessions() as session:
        prior_block = session.scalars(
    ...
    if consume_outcome.verdict == "blocked":
        ...
        # apply_transition never raises: every refusal comes back as an
        # outcome with a refusal receipt code. Only APPLIED or the exact
        # replay of this very command (duplicate=True, the original
        # receipt, §2.6) is an honest block landing; anything else is a
        # composition defect (a missing or non-verified feature row, a
        # version drift) and must surface, never be swallowed.
