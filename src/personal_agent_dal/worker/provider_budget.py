...
def _available(path):
    ...
    if not isinstance(value,dict):raise ValueError('PROVIDER_BUDGET_INVALID')
...
def available(path):
    ...
    try:
        return _available(path)
    except (ValueError, TypeError, KeyError, OSError, sqlite3.Error, SupervisorRefusal):
        return False
