...
This verification does not include replacement of the GitHub remote history. Old clones, caches, or other references may still retain old objects; replacing the branch tip alone is not proof of erasure. Before publication, remote refs and privacy status must be checked and the maintainer must decide whether any further old-object removal is needed. The current conclusion is limited to the known samples, identifiers, and scanning rules above.
