#!/usr/bin/env python3
"""Verify the deterministic-verification machine contract against a frozen authority.

The authority freezes the eleven verification oracles, the request/response
schema bytes, and the registry semantics, so a coherent generator edit cannot
silently redefine the classifier's outcome while keeping its own hashes
internally consistent.  It is never written by ``build_verification_manifests.py``.
"""
...
DEFAULT_AUTHORITY = DEFAULT_DIR / "verification-authority_v1.0.json"
...
GENERATED_ROOT_FIELDS = {
    ...
    "registry": {"schema_version", "operation_spec_id", "command_type",
                 ...
                 "feature_transition_receipt_schema", "registry_stages",
                 "check_stages", "stage_result_fields", "fact_fields",
                 "injected_fields", "failure_to_reason", "reason_to_state",
                 "result_status_enum", "classification_precedence", "block_event",
                 "block_write_set", "verified_write_set", "registry_sha256"},
...
ORACLE_FIELDS = {
    ...
    "expected_report_hash", "expected_state_trace", "expected_event_trace",
    "expected_receipts", "expected_external_effect_trace", "expected_final_snapshot",
    "allowed_write_set", "forbidden_side_effects", "coverage_ref",
}

#: The dimensions the authority re-derives per oracle.  The verification
#: authority freezes the classifier's own `(result_status, failure_class)` pair
#: plus `last_verified_sha` (advance vs preserve) and `report_hash` (the JCS
#: binding of commands + exit codes) — those four ARE the semantics the oracles
#: pin, and the comparator's generic trace check does not cover them.
ORACLE_ENTRY_FIELDS = (
    "test_id", "variant_id", "pre_state",
    "expected_result_status", "expected_failure_class", "expected_last_verified_sha",
    "expected_report_hash", "expected_state_trace", "expected_event_trace",
    "expected_receipts", "expected_final_snapshot", "allowed_write_set",
    "forbidden_side_effects", "coverage_ref",
)


class AuthorityVerificationError(ValueError):
    """Generated verification semantics differ from the frozen authority."""
...
def semantic_counts(registry: dict, entries: dict[str, dict]) -> dict[str, int]:
    ...
    return {
        "verification_variants": len(entries),
        "succeeded_variants": count(
            lambda e: e["expected_result_status"] == "succeeded"),
        "blocked_variants": count(
            lambda e: e["expected_result_status"] == "blocked"),
        "failed_variants": count(
            lambda e: e["expected_result_status"] == "failed"),
        "task_failure_variants": count(
            lambda e: e["expected_failure_class"] == "task_failure"),
        ...
        "verified_variants": count(
            lambda e: e["expected_final_snapshot"]["state"] == "verified"),
        "blocked_test_variants": count(
            lambda e: e["expected_final_snapshot"]["state"] == "blocked_test"),
        ...
        "last_verified_sha_advanced_variants": count(
            lambda e: e["expected_last_verified_sha"] == "0123456789abcdef0123456789abcdef01234567"),
    }
...
def _verify_loaded(
    ...
    if set(authority) != AUTHORITY_KEYS:
        raise AuthorityVerificationError("verification authority root fields are not closed")
    if authority["schema_version"] != "dal.verification-authority/1.0":
        raise AuthorityVerificationError("verification authority schema version mismatch")
    if authority["authority_version"] != "1.0":
        raise AuthorityVerificationError("verification authority version mismatch")
    if authority["source_boundary"] != "static-independent-review-authority; never generated by build_verification_manifests.py":
        raise AuthorityVerificationError("verification authority source boundary mismatch")
    if digest(hashed_payload(authority, "authority_sha256")) != authority["authority_sha256"]:
        raise AuthorityVerificationError("verification authority self-hash mismatch")
    ...
    if authority["semantic_counts"] != counts:
        raise AuthorityVerificationError("verification authority denominator mismatch")
    return counts


def verify_authority(directory: Path = DEFAULT_DIR, authority_path: Path = DEFAULT_AUTHORITY) -> dict[str, int]:
    request_path = directory / "verification-request_schema_v1.0.json"
    response_path = directory / "verification-response_schema_v1.0.json"
    return _verify_loaded(
        ...
        load_json(directory / "verification-registry_v1.0.json"),
        load_json(directory / "verification-fixtures_v1.0.json"),
        load_json(directory / "verification-oracles_v1.0.json"),
        load_json(directory / "verification-manifest_v1.0.json"),
    )
...
def mutation_self_test(directory: Path = DEFAULT_DIR, authority_path: Path = DEFAULT_AUTHORITY) -> list[str]:
    ...
    source = {
        ...
        "registry": load_json(directory / "verification-registry_v1.0.json"),
        "fixtures": load_json(directory / "verification-fixtures_v1.0.json"),
        "oracles": load_json(directory / "verification-oracles_v1.0.json"),
        "manifest": load_json(directory / "verification-manifest_v1.0.json"),
    }
    ...
    def failure_class_mutation(values: dict) -> None:
        oracle(values, "format_fail")["expected_failure_class"] = "contract_failure"
        rehash_chain(values)

    def reason_code_mutation(values: dict) -> None:
        oracle(values, "format_fail")["expected_final_snapshot"]["reason_code"] = \
            "PROVIDER_CONTRACT_FAILURE"
        rehash_chain(values)

    def oracle_write_mutation(values: dict) -> None:
        oracle(values, "all_pass")["allowed_write_set"] = ["aggregate"]
        rehash_chain(values)

    def oracle_status_mutation(values: dict) -> None:
        oracle(values, "test_fail")["expected_result_status"] = "succeeded"
        rehash_chain(values)

    def registry_enum_mutation(values: dict) -> None:
        values["registry"]["reason_to_state"]["TEST_BLOCKED"] = "needs_human"
        rehash_chain(values)

    def last_verified_sha_mutation(values: dict) -> None:
        oracle(values, "all_pass")["expected_last_verified_sha"] = "0" * 40
        rehash_chain(values)

    def report_hash_mutation(values: dict) -> None:
        oracle(values, "all_pass")["expected_report_hash"] = "e" * 64
        rehash_chain(values)
    ...
    for label, mutate, recompute_bytes in (
        ...
        ("last_verified_sha", last_verified_sha_mutation, False),
        ("report_hash", report_hash_mutation, False),
        ...
        try:
            ...
            raise AuthorityVerificationError(f"single-sided {label} mutation escaped verification authority")
    return passed
