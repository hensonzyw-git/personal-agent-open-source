#!/usr/bin/env python3
"""Verify the coder-adapter machine contract against a frozen authority.

The authority freezes the sixteen coder oracles, the request/response schema
...
consistent.  It is never written by ``build_coder_adapter_manifests.py``.
"""
...
DEFAULT_AUTHORITY = DEFAULT_DIR / "coder-adapter-authority_v1.0.json"
...
GENERATED_ROOT_FIELDS = {
    ...
    "registry": {"schema_version", "operation_spec_id", "command_type",
                 ...
                 "feature_transition_receipt_schema", "event_vocabulary",
                 "fact_fields", "injected_fields", "transport_fields",
                 "budget_fields", "failure_class_enum", "failure_to_reason",
                 "reason_to_state", "classification_precedence", "block_event",
                 "block_write_set", "registry_sha256"},
...
ORACLE_FIELDS = {
    ...
    "expected_result_status", "expected_failure_class", "expected_state_trace",
    "expected_event_trace", "expected_receipts",
    "expected_external_effect_trace", "expected_final_snapshot",
    "allowed_write_set", "forbidden_side_effects", "coverage_ref",
}

#: The dimensions the authority re-derives per oracle.  Unlike the
#: controller-dispatch authority, the coder authority freezes the classifier's
#: own `(result_status, failure_class)` pair — that pair IS the semantic the
#: oracles pin, and the comparator's generic trace check does not cover it.
ORACLE_ENTRY_FIELDS = (
...
class AuthorityVerificationError(ValueError):
    """Generated coder-adapter semantics differ from the frozen authority."""
...
def semantic_counts(registry: dict, entries: dict[str, dict]) -> dict[str, int]:
    ...
    return {
        "coder_variants": len(entries),
        "succeeded_variants": count(
            lambda e: e["expected_result_status"] == "succeeded"),
        "cancelled_variants": count(
            lambda e: e["expected_result_status"] == "cancelled"),
        ...
        "usage_limit_variants": count(
            lambda e: e["expected_failure_class"] == "usage_limit"),
        "transient_variants": count(
            lambda e: e["expected_failure_class"] == "transient"),
        "auth_variants": count(
            lambda e: e["expected_failure_class"] == "auth"),
        "budget_limit_variants": count(
            lambda e: e["expected_failure_class"] == "budget_limit"),
...
def _verify_loaded(
    ...
    if set(authority) != AUTHORITY_KEYS:
        raise AuthorityVerificationError("coder authority root fields are not closed")
    if authority["schema_version"] != "dal.coder-adapter-authority/1.0":
        raise AuthorityVerificationError("coder authority schema version mismatch")
    if authority["authority_version"] != "1.0":
        raise AuthorityVerificationError("coder authority version mismatch")
    if authority["source_boundary"] != "static-independent-review-authority; never generated by build_coder_adapter_manifests.py":
        raise AuthorityVerificationError("coder authority source boundary mismatch")
    if digest(hashed_payload(authority, "authority_sha256")) != authority["authority_sha256"]:
        raise AuthorityVerificationError("coder authority self-hash mismatch")
    ...
    if authority["semantic_counts"] != counts:
        raise AuthorityVerificationError("coder authority denominator mismatch")
    return counts


def verify_authority(directory: Path = DEFAULT_DIR, authority_path: Path = DEFAULT_AUTHORITY) -> dict[str, int]:
    request_path = directory / "coder-adapter-request_schema_v1.0.json"
    response_path = directory / "coder-adapter-response_schema_v1.0.json"
    return _verify_loaded(
        ...
        load_json(directory / "coder-adapter-registry_v1.0.json"),
        load_json(directory / "coder-adapter-fixtures_v1.0.json"),
        load_json(directory / "coder-adapter-oracles_v1.0.json"),
        load_json(directory / "coder-adapter-manifest_v1.0.json"),
    )
...
def mutation_self_test(directory: Path = DEFAULT_DIR, authority_path: Path = DEFAULT_AUTHORITY) -> list[str]:
    ...
    source = {
        ...
        "registry": load_json(directory / "coder-adapter-registry_v1.0.json"),
        "fixtures": load_json(directory / "coder-adapter-fixtures_v1.0.json"),
        "oracles": load_json(directory / "coder-adapter-oracles_v1.0.json"),
        "manifest": load_json(directory / "coder-adapter-manifest_v1.0.json"),
    }
    ...
    def failure_class_mutation(values: dict) -> None:
        oracle(values, "tampered_host")["expected_failure_class"] = "contract_failure"
        rehash_chain(values)

    def reason_code_mutation(values: dict) -> None:
        oracle(values, "tampered_host")["expected_final_snapshot"]["reason_code"] = \
            "PROVIDER_CONTRACT_FAILURE"
        rehash_chain(values)

    def oracle_write_mutation(values: dict) -> None:
        oracle(values, "happy_multi_tool")["allowed_write_set"] = ["aggregate"]
        rehash_chain(values)

    def oracle_status_mutation(values: dict) -> None:
        oracle(values, "cancel")["expected_result_status"] = "succeeded"
        rehash_chain(values)

    def registry_enum_mutation(values: dict) -> None:
        values["registry"]["failure_class_enum"].append("task_failure")
        rehash_chain(values)

    def schema_enum_mutation(values: dict) -> None:
        values["response_schema"]["properties"]["failure_class"]["enum"].append("task_failure")
    ...
    for label, mutate, recompute_bytes in (
        ...
        try:
            ...
            raise AuthorityVerificationError(f"single-sided {label} mutation escaped coder authority")
    return passed
