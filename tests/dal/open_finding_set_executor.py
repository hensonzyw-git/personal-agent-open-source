"""Executor for `DAL-T-OPENSET-001` fixture bodies.

Replays the `derive_open_finding_set` operation command under the pure policy
guard and maps the handler's frozen result onto the generic `ExecutionTrace`.
The two-action group is consumed as one operation by the handler (derivation
...
from personal_agent_dal.machine.open_finding_set import derive_open_finding_set
