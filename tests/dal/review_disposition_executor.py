"""Executor for `DAL-T-DISPOSITION-001` fixture bodies.

Replays the `recompute_review_disposition` operation command under the pure
...
handler (recompute precedes the record inside it), so the executor dispatches
exactly one call.
"""
...
from personal_agent_dal.machine.review_disposition import recompute_review_disposition
